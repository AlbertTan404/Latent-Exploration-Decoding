...
from led.decoding import get_global_decoding_params, latent_exploration_decoding, dola_decoding
...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        """
        #++
        global_decoding_params = get_global_decoding_params()
        if global_decoding_params.method:
            if enable_think_only:
                soft_mask = sampling_info.soft_thinking_modes # Shape (B,)
            else:
                soft_mask = torch.ones(size=(logits_output.next_token_logits.shape[0], ), dtype=torch.bool, device=logits_output.next_token_logits.device) # Shape (B,)
            if global_decoding_params.method == 'led':
                return latent_exploration_decoding(
                    logits_output=logits_output,
                    decoding_params=global_decoding_params,
                    is_all_greedy=sampling_info.is_all_greedy, temperatures=sampling_info.temperatures,
                    think_only_mask=soft_mask.unsqueeze(1)
                )
            elif global_decoding_params.method == 'dola':
                return dola_decoding(
                    logits_output=logits_output,
                    decoding_params=global_decoding_params,
                    is_all_greedy=sampling_info.is_all_greedy, temperatures=sampling_info.temperatures, topks=sampling_info.top_ks, topps=sampling_info.top_ps
                )
            else:
                raise NotImplementedError(f"Decoding method {global_decoding_params.method} is not implemented yet.")
        ...
        if sampling_info.is_all_greedy:  # avoid entering greedy sampling logic
            ...
            if enable_soft_thinking:
                # Hard aligning with ST's settings
                logits.div_(0.6)
                #
                ...
                if any(non_soft_mask):
                    ...
                    topk_indices[non_soft_mask, 0] = batch_next_token_ids[non_soft_mask].view(-1)
            ...
            if global_server_args_dict["sampling_backend"] == "flashinfer":
                ...
                if enable_soft_thinking:
                    ...
                    non_soft_mask = ~soft_mask
                    if any(non_soft_mask):
                        # print('after_soft_thinking triggered', flush=True)
